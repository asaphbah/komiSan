...
<head>
  ...
  <title>Formulário de Report de Tags</title>
  <style>
...
<body class="body-cadastro">
    <div class="container-cadastro-report">
        <form class="cadastro-report" method="POST" action="{{ route('report.tag.store') }}">
        @csrf
        <h1>Reporte de Tag</h1>
        <input type="hidden" name="tag_id" value="{{$tag_id}}">
        <input type="hidden" name="user_id" value="{{ auth()->user()->id }}">
        <div class="input-box input-report">
            <input  type="text" name="report_text" placeholder="Justifique o report da tag">
        </div>
